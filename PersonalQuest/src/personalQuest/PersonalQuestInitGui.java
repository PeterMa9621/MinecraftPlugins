...
public class PersonalQuestInitGui 
{
	...
	public Inventory initConfirmForPlayer(Player p1, Player p2, String questType, String posterName)
	{
				
		Inventory inv = Bukkit.createInventory(p1, 54, "§a你正与§c"+p2.getName()+"§a确认任务");
		
		ItemStack window = plugin.createItem(160, 1, 0, "§6请放入要提交的任务物品或要奖励的物品");
		for(int i=18; i<36; i++)
		{
		...
		/*
		int i=0;
		if(plugin.comfirmGuiItems.containsKey(p2.getName()))
		{
			for(ItemStack item:plugin.comfirmGuiItems.get(p2.getName()))
			{
				inv.setItem(i, item);
				i+=1;
			}
		...
		*/
		ItemStack confirmButton = plugin.createItem(Material.SLIME_BALL, plugin.message.get("ConfirmGui.ConfirmButton1"));

		inv.setItem(17, confirmButton);
		
		
		ItemStack infoButton = plugin.createItem(Material.CLAY, "§3任务类型:"+questType+",任务发起者:"+posterName);
	...
	public Inventory initMyQuest(Player p)
	{
		...
		if(!plugin.playerQuest.get(p.getName()).isEmpty())
		{
			for(ArrayList<String> everyQuest:plugin.playerQuest.get(p.getName()))
			{
				...
				{
					lore += "%§6悬赏§c"+everyQuest.get(3)+"§6的头颅";
				}
	...
	public ArrayList<Inventory> initQuestGUI(Player p)
	{
		...
		{
			if(!plugin.headQuests.get(player).isEmpty())
			{
				ArrayList<String> result = plugin.headQuests.get(player).get("quest");
				
				//result.add(plugin.headQuests.get(player).get("quest").get(plugin.headQuests.get(player).get("quest").size()-1));
		...
		for(int j=0; j<totalSize/45+1; j++)
		{
			...
			{
				if(!plugin.vipHeadQuests.get(player).isEmpty())
				{
					ArrayList<String> result = plugin.vipHeadQuests.get(player).get("quest");
					
					//result.add(plugin.vipHeadQuests.get(player).get("quest").get(plugin.vipHeadQuests.get(player).get("quest").size()-1));
			...
			for(ArrayList<String> vip:vipQuestList)
			{
				...
				if(type.equalsIgnoreCase("VIPhead"))
				{
					lore.add("§6悬赏§c"+vip.get(3)+"§6的头颅");
					if(plugin.needToBeKilled!=null)
					{
						if(!plugin.needToBeKilled.contains(vip.get(3)))
						{
							lore.add("§a已有人击杀了"+vip.get(3));
						}
					}
			...
			if(size>0)
			{
				...
				for(int x=0; x<36; x++)
				{
					...
					if(type.equalsIgnoreCase("head"))
					{
						lore.add("§6悬赏§c"+i.get(3)+"§6的头颅");
						if(plugin.needToBeKilled!=null)
						{
							if(!plugin.needToBeKilled.contains(i.get(3)))
							{
								lore.add("§c已有人击杀了"+i.get(3));
							}
						}
					}
