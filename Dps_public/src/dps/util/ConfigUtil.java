...
public class ConfigUtil {
    public static void loadConfig(Dps plugin) {
        ...
        if(!file.exists()){
            ...
            config.set("expCommand", "xp give %player% %amount%");
            try {
                config.save(file);
            } catch (IOException e) {
                e.printStackTrace();
        ...
        RewardBoxManager.expCommand = config.getString("expCommand", "");
    ...
    public static void loadNormalRewardConfig(Dps plugin)
    {
        ...
        FileConfiguration config;
        if (!file.exists()) {
            Bukkit.getConsoleSender().sendMessage("§6[Dps] §c没有item文件存在!");
            config = load(file);
            config.set("rewards", new ArrayList<String>() {{
                add("eco give %player% 100 #DIAMOND#钻石");
            }}.toArray());
        ...
        for(String rewardCmd:config.getStringList("rewards")) {
            if(!rewardCmd.contains("#")) {
                Bukkit.getConsoleSender().sendMessage("§6[Dps] §c普通奖励物品格式错误!");
                continue;
            }
